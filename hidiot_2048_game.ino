...
const int THRESHOLD  PROGMEM = 980;    // analogue input threshold
...
void setup()
{
   // turn the LED on when starting up
...
void loop()
{
   ...
   if(input < THRESHOLD) {
      ...
      if(input < THRESHOLD) {
         input_present = true;
         digitalWrite(redLED, LOW);       // turn the LED off for now
      }
   }
   else {
      if(input_present) {
         ...
         input_present = false;
         digitalWrite(redLED, HIGH);      // turn the LED back on
